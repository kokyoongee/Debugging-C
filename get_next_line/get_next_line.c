...
char	*ft_read_to_left_str(int fd, char *left_str)
{
	...
	{
		rd_bytes = read(fd, buff, BUFFER_SIZE); // assigned read bytes from read function
		if (rd_bytes == -1)
		{
			free(buff);
			return (NULL);
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	left_str = ft_read_to_left_str(fd, left_str);// it reads file from fd and stops when \n or EOF is encountered and return the read_str.
	if (!left_str)
		return (NULL);
